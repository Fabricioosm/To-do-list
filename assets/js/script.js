...
const btnCriar = document.querySelector('.btn-created')
const notList = document.querySelector('.container-tasks')

const countercompleted = document.querySelector('.counter-completed')
const counterCreated = document.querySelector('.counter-created')
...
function createdList(){
    ...
    let counterNum = Number(counterCreated.innerText)
    counterCreated.innerText = counterNum + 1
    ...
    textList.classList.add('txt-list-item')
    textList.innerText = inputList.value


    const btnTrash = document.createElement('button')
    btnTrash.innerHTML = `<svg width="13" height="14" viewBox="0 0 13 14" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    btnTrash.addEventListener('click', function taskDelete(){
        if (counterCreated.innerText === '1'){
            notList.style.display = 'flex'
        }
    ...
    novoItem.appendChild(textList)
    novoItem.appendChild(btnTrash)

    listaItens.appendChild(novoItem)

    cardContainer.appendChild(listaItens)
...
function countertasks(){
    ...
    counterCreated.innerText = total
    countercompleted.innerText = concluidas + " de " + total
}
